...
from bson import ObjectId  # <-- NEW: to coerce string ids for Mongo lookups
...
def _maybe_oid(x):
    """Coerce possible string ObjectId to real ObjectId for Mongo queries."""
    try:
        return ObjectId(x)
    except Exception:
        return x
...
def _assert_results_exist_in_articles(client, results_list):
    ...
    for r in results_list:
        ...
        doc = articles.find_one({"_id": _maybe_oid(aid)}, {"_id": 1})   # <-- updated
...
def test_llm_graph_author_from_context_one():
    ...
    if result.get("top_results"):
        _assert_results_exist_in_articles(client, result["top_results"])
        ra = result["top_results"][0].get("author")
        if isinstance(ra, list):
            assert author in ra
        else:
            assert ra == author
...
# 6) user asks for articles from the last 2 months (relative dates)
